...
def architecture(dll):
    ...
    output = subprocess.check_output([args.dumpbin, "/HEADERS", dll])
...
def extract_symbols(dll):
    output = subprocess.check_output([args.dumpbin, "/EXPORTS", dll])
    output = output.decode("utf-8")
    lines = output.split("\r\n")
    start, end = None, None
    start = (
        next(
            ...
            if all(
                map(lambda entry: (entry in line), ["ordinal", "hint", "RVA", "name"])
            )
        )
        + 2
    )
...
def undecorate(names):
    ...
    try:
        with open(random_filename, "w+t") as f:
            f.write("\n".join(names))
        demangled = subprocess.check_output([args.undname, random_filename])
    except:
        raise
    finally:
...
if __name__ == "__main__":
    ...
    with open("def_template") as def_template_file:
        def_template = Template(
            def_template_file.read(), trim_blocks=True, lstrip_blocks=True
        )

    with open("cpp_template") as cpp_template_file:
        cpp_template = Template(
            cpp_template_file.read(), trim_blocks=True, lstrip_blocks=True
        )

    with open("asm_template") as asm_template_file:
        asm_template = Template(
            asm_template_file.read(), trim_blocks=True, lstrip_blocks=True
        )

    with open("cmake_template") as cmake_template_file:
        cmake_template = Template(
            cmake_template_file.read(), trim_blocks=True, lstrip_blocks=True
        )
    ...
    cpp_content = cpp_template.render(
        dll=dll, architecture=arch, hook=args.hook, ordinal_and_names=ordinal_and_names
    )
